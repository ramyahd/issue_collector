...
String response1 = sh(script:"""curl --location --request POST 'http://18.221.205.57:8181/v1/data/sonarrbac/policy/allow' --header 'Content-Type: application/json' --data-raw '{ "input" : { "user": "alice","branch": "master","auth": "ram"} }'""", returnStdout: true)
println(response1)
  
String response2 = sh(script:"""curl --location --request POST 'http://18.221.205.57:8181/v1/data/sonarrbac/policy/result' --header 'Content-Type: application/json' --data-raw '{ "input" : { "user": "alice","branch": "master", "auth": "ram"} }'""", returnStdout: true)
println(response2)  
}
